...
public class AnimationDotFootstepParticle {
	...
	public static void registerParticleType(RegistryEvent.Register<ParticleType<?>> event) {
		event.getRegistry().register(particle.setRegistryName("animation_dot_footstep"));
	}
	...
	private static class CustomParticle extends SpriteTexturedParticle {
		...
		public void tick() {
			super.tick();
			if (!this.isExpired) {
				this.setSprite(this.spriteSet.get((this.age / 100) % 4 + 1, 4));
			}
		}
	}
