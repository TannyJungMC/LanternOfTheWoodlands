...
public class Spell4GUIbuttonBowActivateProcedure {
		...
		if (!((entity.getCapability(LanternofthewoodlandsModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new LanternofthewoodlandsModVariables.PlayerVariables())).user_DELETE_THIS).equals("")
				&& (entity.getCapability(LanternofthewoodlandsModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new LanternofthewoodlandsModVariables.PlayerVariables())).spell == 4) {
			{
				...
				for (Entity entityiterator : _entfound) {
					if ((entity.getCapability(LanternofthewoodlandsModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new LanternofthewoodlandsModVariables.PlayerVariables())).spell4_weapon_bow_count_max > (entity
							.getCapability(LanternofthewoodlandsModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new LanternofthewoodlandsModVariables.PlayerVariables())).spell4_weapon_bow_count) {
						from_item = "minecraft:bow";
						to_item = "bow";
						to_item_id = "minecraft:bow";
						if ((ForgeRegistries.ITEMS.getKey((entityiterator instanceof ItemEntity _itemEnt ? _itemEnt.getItem() : ItemStack.EMPTY).getItem()).toString()).equals(from_item)) {
							{
								double _setval = (entity.getCapability(LanternofthewoodlandsModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new LanternofthewoodlandsModVariables.PlayerVariables())).spell4_weapon_bow_count + 1;
								entity.getCapability(LanternofthewoodlandsModVariables.PLAYER_VARIABLES_CAPABILITY, null).ifPresent(capability -> {
									capability.spell4_weapon_bow_count = _setval;
							...
							if (true) {
								{
									...
									if (!_ent.level().isClientSide() && _ent.getServer() != null) {
										_ent.getServer().getCommands().performPrefixedCommand(
												...
												(("summon armor_stand ~ ~ ~ {Tags:[\"LOTW\",\"<ID>\",\"LOTW-spell4_" + "" + to_item + "\"],Invisible:1b,Marker:1b,NoGravity:1b,ShowArms:1b,CustomName:'{\"text\":\"LOTW-spell4\"}',HandItems:[{id:\""
														+ to_item_id + "\",Count:1b},{}]}")
								...
								}
								{
									...
									if (!_ent.level().isClientSide() && _ent.getServer() != null) {
										_ent.getServer().getCommands().performPrefixedCommand(
												new CommandSourceStack(CommandSource.NULL, _ent.position(), _ent.getRotationVector(), _ent.level() instanceof ServerLevel ? (ServerLevel) _ent.level() : null, 4, _ent.getName().getString(),
														_ent.getDisplayName(), _ent.level().getServer(), _ent),
												(("data modify entity @e[tag=LOTW-spell4_" + to_item + ",tag=<ID>,limit=1,distance=..0.01] ArmorItems[0] set from entity @s Item").replace("<ID>",
