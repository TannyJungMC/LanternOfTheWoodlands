...
import lotw.procedures.BagPlaceProcedure;

import lotw.itemgroup.LotwTabItemGroup;

import lotw.gui.BagGUIGuiWindow;
...
public class BagItem extends LotwModElements.ModElement {
	@ObjectHolder("lotw:bag")
	public static final Item block = null;

	public BagItem(LotwModElements instance) {
		super(instance, 582);
	...
	public void onItemDropped(ItemTossEvent event) {
		if (event.getEntityItem().getItem().getItem() == block) {
			if (Minecraft.getInstance().currentScreen instanceof BagGUIGuiWindow) {
	...
	public static class ItemCustom extends Item {
		...
		public void addInformation(ItemStack itemstack, World world, List<ITextComponent> list, ITooltipFlag flag) {
			...
			list.add(new StringTextComponent("> Right click to open inventory"));
			list.add(new StringTextComponent("> Hold shift and right click on ground to place it"));
			list.add(new StringTextComponent("> Left click at bag on ground to pick it up"));
			list.add(new StringTextComponent("> Cancel this user pack will remove all items in the bag"));
			list.add(new StringTextComponent("------------------------------"));
		}
			...
			if (entity instanceof ServerPlayerEntity) {
				NetworkHooks.openGui((ServerPlayerEntity) entity, new INamedContainerProvider() {
					...
					public Container createMenu(int id, PlayerInventory inventory, PlayerEntity player) {
						...
						return new BagGUIGui.GuiContainerMod(id, inventory, packetBuffer);
					}
				}, buf -> {
		...
		public ActionResultType onItemUseFirst(ItemStack stack, ItemUseContext context) {
			...
			BagPlaceProcedure.executeProcedure(Stream
	...
	private static class InventoryCapability implements ICapabilitySerializable<CompoundNBT> {
		...
		private ItemStackHandler createItemHandler() {
			return new ItemStackHandler(65) {
				@Override
