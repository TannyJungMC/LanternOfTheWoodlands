...
import lantern_of_the_woodlands.gui.BasicSpell13bagGuiWindow;
import lantern_of_the_woodlands.gui.BasicSpell13bagGui;
...
public class BagItem extends LanternOfTheWoodlandsModElements.ModElement {
	@ObjectHolder("lantern_of_the_woodlands:bag")
	public static final Item block = null;

	public BagItem(LanternOfTheWoodlandsModElements instance) {
		super(instance, 582);
	...
	public void onItemDropped(ItemTossEvent event) {
		if (event.getEntityItem().getItem().getItem() == block) {
			if (Minecraft.getInstance().currentScreen instanceof BasicSpell13bagGuiWindow) {
	...
	public static class ItemCustom extends Item {
		...
		public void addInformation(ItemStack itemstack, World world, List<ITextComponent> list, ITooltipFlag flag) {
			...
			list.add(new StringTextComponent("> right click to open bag"));
			list.add(new StringTextComponent("> cancel this spell will remove all items in the bag"));
			list.add(new StringTextComponent("--------------------"));
		}
			...
			if (entity instanceof ServerPlayerEntity) {
				NetworkHooks.openGui((ServerPlayerEntity) entity, new INamedContainerProvider() {
					...
					public ITextComponent getDisplayName() {
						return new StringTextComponent("Plant Hunter's Bag");
					}
					...
					public Container createMenu(int id, PlayerInventory inventory, PlayerEntity player) {
						...
						return new BasicSpell13bagGui.GuiContainerMod(id, inventory, packetBuffer);
					}
				}, buf -> {
	...
	private static class InventoryCapability implements ICapabilitySerializable<CompoundNBT> {
		...
		private ItemStackHandler createItemHandler() {
			return new ItemStackHandler(159) {
				@Override
