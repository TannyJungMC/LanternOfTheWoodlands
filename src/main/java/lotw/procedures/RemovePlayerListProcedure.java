	...
	public static void executeProcedure(Map<String, Object> dependencies) {
		if (dependencies.get("world") == null) {
			if (!dependencies.containsKey("world"))
				LotwMod.LOGGER.warn("Failed to load dependency world for procedure RemovePlayerList!");
			return;
		}
		if (dependencies.get("x") == null) {
			if (!dependencies.containsKey("x"))
				LotwMod.LOGGER.warn("Failed to load dependency x for procedure RemovePlayerList!");
			return;
		}
		if (dependencies.get("y") == null) {
			if (!dependencies.containsKey("y"))
				LotwMod.LOGGER.warn("Failed to load dependency y for procedure RemovePlayerList!");
			return;
		}
		if (dependencies.get("z") == null) {
			if (!dependencies.containsKey("z"))
				LotwMod.LOGGER.warn("Failed to load dependency z for procedure RemovePlayerList!");
			return;
		}
			...
			if (world instanceof ServerWorld) {
				((World) world).getServer().getCommandManager().handleCommand(
						...
						("execute if entity " + ("@e[type=" + LotwModVariables.MapVariables.get(world).player_list_manual + ",tag=LOTW-ally]")
								...
								+ " [\"\",{\"text\":\"\",\"color\":\"gold\",\"extra\":[{\"selector\":\"@p[tag=LOTW-user]\"}]},{\"text\":\" removed \",\"color\":\"gold\"},{\"text\":\"\",\"color\":\"gray\",\"extra\":[{\"selector\":\""
								+ ("@e[type=" + LotwModVariables.MapVariables.get(world).player_list_manual + ",tag=LOTW-ally]")
								+ "\"}]},{\"text\":\" from lantern ally list\",\"color\":\"gold\"}]"));
			}
		}
