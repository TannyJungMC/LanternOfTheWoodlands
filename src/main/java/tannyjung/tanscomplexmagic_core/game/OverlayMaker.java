...
public class OverlayMaker {
    ...
    public static void drawText (GuiGraphics graphic, int screen_width, int screen_height, String pos_anchor, int posX, int posZ, double scale, String text) {

        int[] pos = convertPos(screen_width, screen_height, posX, posZ, pos_anchor, scale);
        posX = pos[0];
        posZ = pos[1];
        ...
        */
        graphic.pose().pushPose();
        graphic.pose().scale((float) scale, (float) scale, 1.0f);
        graphic.drawString(Minecraft.getInstance().font, text, posX, posZ, 0, false);
        graphic.pose().popPose();
    ...
    private static int[] convertPos (int screen_width, int screen_height, int posX, int posZ, String pos_anchor, double scale) {
